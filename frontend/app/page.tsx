    ...
    try {
      ...
      if (!res.ok) {
        ...
        try {
          ...
          throw new Error(
            (j && (j.detail || j.message)) || text || `HTTP ${res.status}`
          );
        } catch {
  ...
  return (
    <main className="mx-auto max-w-3xl p-6 space-y-6">
      ...
      <section className="grid gap-4 sm:grid-cols-2">
        <div className="space-y-2">
          <label className="text-sm font-medium">
            Custom labels (add comma-separated labels if needed)
          </label>
          <textarea
            value={labels}
            onChange={(e) => setLabels(e.target.value)}
            className="w-full min-h-[100px] rounded-md border p-3 focus:outline-none focus:ring resize-y"
      ...
      <section className="flex flex-wrap items-center justify-center gap-4">
        <button
          onClick={classify}
          className="w-56 rounded-md bg-indigo-600 px-4 py-2 text-white disabled:opacity-60"
          disabled={loading}
        >
